...
function AdminEvents() {
  ...
  // load all events
  useEffect(()=>{
    fetch("https://college-erp-backend-360.onrender.com/events/everyone")
      .then(res=>res.json())
  ...
  const upload=()=>{
    ...
    fetch("https://college-erp-backend-360.onrender.com/add_event",{
      method:"POST",
      body:formData
    })
    .then(res=>res.json())
  ...
  const remove=(id)=>{
    ...
    fetch(`https://college-erp-backend-360.onrender.com/delete_event/${id}`,{
      method:"DELETE"
    })
    .then(res=>res.json())
    .then(()=>{
